...
class ParameterFitter:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'ParameterFitter_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Parameter Fitter')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('ParameterFitter', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/parameter_fitter/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Parameter Fitter'),
            callback=self.run,
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = ParameterFitterDialog()
        ...
        if result:
            ...
            if len(data_path) == 0:
                QMessageBox.information(None, "!!!ERROR!!!", "No File Provided") 
                self.iface.addVectorLayer(stop_program) 

            # get combo box input
            model_type = self.dlg.model_type.currentText()
            if len(model_type) == 0:
                QMessageBox.information(None, "!!!ERROR!!!", "No Model Type Selected") 
                self.iface.addVectorLayer(stop_program) 
